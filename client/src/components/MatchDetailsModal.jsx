    ...
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-900 rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex justify-between items-center p-6 border-b border-gray-800">
          <h2 className="text-white text-2xl font-semibold">{match.name}</h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6">
          {/* Opis */}
          <div className="mb-6">
            <h3 className="text-white text-lg font-medium mb-2">Opis</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
            ...
            <div className="flex items-center text-gray-300">
              ...
              <div>
                <p className="text-sm text-gray-400">Godzina</p>
            ...
            <div className="flex items-center text-gray-300">
              ...
              <div>
                <p className="text-sm text-gray-400">Uczestnicy</p>
          ...
          {/* Lista uczestników */}
          <div>
            <h3 className="text-white text-lg font-medium mb-4">Uczestnicy</h3>

            {participants.length === 0 ? (
              <p className="text-gray-400">Brak potwierdzonych uczestników</p>
            ) : (
                ...
                {sortedPositions.map((position) => (
                  <div key={position} className="bg-gray-800 rounded-lg p-4">
                    <h4 className="text-white font-medium mb-2 capitalize">
                      ...
                      {position === "obrońca" && "Obrońcy"}
                      {position === "pomocnik" && "Pomocnicy"}
                      {position === "napastnik" && "Napastnicy"}
                      {position === "Brak pozycji" && "Bez określonej pozycji"}
                    </h4>
                    <ul className="space-y-2">
                      {groupedParticipants[position].map((participant) => (
                        <li key={participant.id} className="flex items-center text-gray-300">
        ...
        <div className="p-6 border-t border-gray-800 text-right">
          <button
            onClick={onClose}
            className="px-5 py-2 bg-white text-black rounded-md font-medium hover:bg-gray-200 transition-colors"
          >
            Zamknij
          </button>
        </div>
