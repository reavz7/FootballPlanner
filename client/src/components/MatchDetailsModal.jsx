...
const MatchDetailsModal = ({ match, participants, onClose }) => {
  ...
  const positionOrder = ["bramkarz", "obrońca", "pomocnik", "napastnik", "Brak pozycji"]
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50 p-4 bg-transparent">
      <div className="bg-white dark:bg-gray-900 rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-lg">
        ...
        <div className="flex justify-between items-center p-6 border-b border-gray-200 dark:border-gray-800">
          <h2 className="text-gray-900 dark:text-white text-2xl font-semibold">{match.name}</h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6">
          {/* Opis */}
          <div className="mb-6">
            <h3 className="text-gray-900 dark:text-white text-lg font-medium mb-2">Opis</h3>
            <p className="text-gray-700 dark:text-gray-300">{match.description}</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
            <div className="flex items-center text-gray-700 dark:text-gray-300">
              ...
              <div>
                ...
                <p className="text-gray-900 dark:text-white">{match.location}</p>
              </div>
            </div>
            ...
            <div className="flex items-center text-gray-700 dark:text-gray-300">
              ...
              <div>
                ...
                <p className="text-gray-900 dark:text-white">{formatDate(match.date)}</p>
              </div>
            </div>
            ...
            <div className="flex items-center text-gray-700 dark:text-gray-300">
              ...
              <div>
                <p className="text-sm text-gray-500 dark:text-gray-400">Godzina</p>
            ...
            <div className="flex items-center text-gray-700 dark:text-gray-300">
              ...
              <div>
                <p className="text-sm text-gray-500 dark:text-gray-400">Uczestnicy</p>
          ...
          {/* Lista uczestników */}
          <div>
            <h3 className="text-gray-900 dark:text-white text-lg font-medium mb-4">Uczestnicy</h3>

            {participants.length === 0 ? (
              <p className="text-gray-500 dark:text-gray-400">Brak potwierdzonych uczestników</p>
            ) : (
              <div className="space-y-4">
                {sortedPositions.map((position) => (
                  <div key={position} className="bg-gray-100 dark:bg-gray-800 rounded-lg p-4">
                    ...
                    <ul className="space-y-2">
                      {groupedParticipants[position].map((participant) => (
                        <li key={participant.id} className="flex items-center text-gray-700 dark:text-gray-300">
                          <User className="h-4 w-4 mr-2 text-gray-500 dark:text-gray-400" />
                          <span>{participant.User?.username || "Użytkownik"}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
